...
func main() {
	...
	for _, slice := range config.Slices {
		...
		files[projectorFile].Type().Id("projector").StructFunc(func(group *Group) {
			group.Add(Id("eventbus").Qual("github.com/ThreeDotsLabs/watermill/components/cqrs", "EventBus"))
		})

		files[projectorFile].Func().Id("New").ParamsFunc(func(group *Group) {
			group.Add(Id("eventbus").Qual("github.com/ThreeDotsLabs/watermill/components/cqrs", "EventBus"))
		}).Params(Id("Service")).Block(Return().Op("&").Id("projector").Block(DictFunc(func(dict Dict) {
