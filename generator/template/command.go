...
func (t *commandTemplate) Render(ctx context.Context) write_strategy.Renderer {
	...
	if len(t.info.Slice.Specifications) > 0 {
	...
	z.Line().Func().Id("evolve").Params(Id("state").Id(stateName), Id("envelope").Op("*").Qual(PackageEventSourcing, "Envelope")).Params(Id(stateName)).BlockFunc(func(group *Group) {
	...
	z.Line().Func().Id("decide").Params(Id("state").Id(stateName), Id("cmd").Id(commandName)).Params(Index().Qual(PackageEventSourcing, "Event"), Error()).BlockFunc(func(group *Group) {
	...
	z.Func().Id("New" + commandName + "Handler").Params(Id("eventStore").Qual(PackageEventSourcing, "EventStore")).Params(Func().Params(
		...
		group.Return(Qual(PackageEventSourcing, "NewCommandHandler").Call(
			Line().Id("eventStore"),
			Line().Id("initialState"),
			Line().Id("evolve"),
			Line().Id("decide"),
			Line().Qual(PackageEventSourcing, "WithStreamState").Call(Qual(PackageEventSourcing, "Any").Block()),
		))
	})
